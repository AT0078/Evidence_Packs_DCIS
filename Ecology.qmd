...
############### Inver has coords, Invert_Site has everything we need but no spatial data.
...
#Load in site coords
Site_Coords <- read.csv("/dbfs/FileStore/WSX_HGray/ETL_Exports/Sites.csv") %>% 
                          rename_with(tolower) %>% 
...
Inv_Site_22 <- inner_join(Inv_Site, Site_Coords, by = "Site.ID") %>% 
...
leaflet() %>% 
    ...
    addCircleMarkers(data = ARMI,
                   ...
                   popup = paste0("Mean Total Score: ", ARMI$Mean_Tot,  
                                  ...
                                  "<br> Group: ", ARMI$Team,
                                  "<br> From/ To: ", min(ARMI$Recorded__Date), "/ ", max(ARMI$Recorded..Date)),
                    options = pathOptions(zIndex = 999)) %>% 
