...
The below map provides the whereabouts of continuous permitted WWtW and their likelihood of eutrophication, as well as the option to view intermittent CSO discharges for 2024. These provide some indication of the loading from point source pollution on waterbodies. Point source pollution from fish farms and private consented discharges are not mapped here but can be found on easimap.
...
WWTW <- readxl:: read_excel("/dbfs/mnt/lab/unrestricted/harry.gray@environment-agency.gov.uk/ETL_Exports/STW_spreadsheet_DRAFT.xlsx", sheet= "MASTER", skip=1)
...
sewage <- readxl::read_excel("/dbfs/mnt/lab/unrestricted/harry.gray@environment-agency.gov.uk/ETL_Exports/EDM_2024_Storm_Overflow_Annual_Return___all_water_and_sewerage_companies.xlsx", sheet="Wessex Water 2024", skip=1)
...
# Crop sf
 ...
 # Convert 1899-12-30 to POSIXct
  ...
  sewage %<>% 
    mutate(
      Total_spill_hrs_2024 = as.numeric(sewage$`Total Duration (hh:mm:ss) all spills prior to processing through 12-24h count method` - origin, units = "hours")
          )

# Could add private discharges as here we only look at water company discharges
...
PAL <- colorNumeric("YlOrBr", domain = sewage$Total_spill_hrs_2024)


sequ <- seq(min(sewage$Total_spill_hrs_2024, na.rm = TRUE), 
            max(sewage$Total_spill_hrs_2024, na.rm = TRUE), length.out = 5)
...
leaflet() %>% 
  ...
  addCircleMarkers(data=sewage,
                      radius = ~(sqrt(Total_spill_hrs_2024)-1), 
                      color = "black",
                      weight = 1,
                      fillColor = ~PAL(Total_spill_hrs_2024),
                      fillOpacity = 1,
                      popup = ~paste0("Annual Spill Count (2023): ", as.character(round(Total_spill_hrs_2024), 0), " hrs", "<br> At site: ", `Site Name\r\n(EA Consents Database)`),
                   options = pathOptions(zIndex = 600),
  ...
  addLegend("bottomright", 
                      pal = PAL, 
                      values = sequ, 
                      title = "Sewage Discharge <br> Total Hrs 2024",
                      group = "WxW CSO Sewage Discharge Points") %>% 
                    addScaleBar()  %>% 
...
# This esri geodatabase file is acquired by emailing Crispin Hambidge, the data custodian of ALERT data
gdb <- st_read("/dbfs/mnt/lab/unrestricted/harry.gray@environment-agency.gov.uk/ETL_Exports/WessexRiskAttributedOLF.gdb", quiet = TRUE)
        ...
        #       color = ~pal_ALERT(CatchmentRiskScore),
         #      opacity = 1,
          ...
          addLayersControl(baseGroups = c("Esri Basemap", "Satellite Basemap"),
            overlayGroups = c("Catchment Risk"),
                           position = "topright",
                           options = layersControlOptions(collapsed = FALSE)) %>% 
...
Phys_Chem_Site <- readxl::read_excel("/dbfs/mnt/lab/unrestricted/harry.gray@environment-agency.gov.uk/ETL_Exports/2022_Phys_Chem_Classification_QA_document__RAW_.xlsx", sheet= "Site Class")
...
```{r BOD 22-25 Monitoring Mapping}
            ...
            leaflet() %>% 
              ...
              addCircleMarkers(data= df,
                               ...
                               popup = paste0("Site: ",df$Site_Name,"<br> ",
                                              "Mean Nitrate (ppm): ",round(df$Mean_Nitrate,2),
                                              "<br> Sampling Date Range: ", paste0(range(df$Date)[1], " to ", 
                                               range(df$Date)[2],
